...
variable "vpb_installer_path" {
  type    = string
  default = "/Users/brinketu/CloudlensFIle/vpb-3.7.0-34-install-package.sh"
}
...
resource "azurerm_subnet" "auto_vpb_subnet_main" {
  ...
  address_prefixes     = ["172.24.7.0/24"]
}
...
resource "azurerm_subnet" "auto_vpb_subnet_eth1" {
  ...
  resource_group_name  = data.azurerm_resource_group.existing.name
  virtual_network_name = data.azurerm_virtual_network.existing_vnet.name
  address_prefixes     = ["172.24.8.0/24"]
}
...
resource "azurerm_subnet" "auto_vpb_subnet_eth2" {
  ...
  resource_group_name  = data.azurerm_resource_group.existing.name
  virtual_network_name = data.azurerm_virtual_network.existing_vnet.name
  address_prefixes     = ["172.24.9.0/24"]
}
