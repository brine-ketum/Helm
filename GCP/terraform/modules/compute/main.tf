...
resource "google_compute_instance" "instances" {
  ...
  boot_disk {
    initialize_params {
      # Fix: Build the image path correctly
      image = lookup(each.value, "source_image", "") != "" ? lookup(each.value, "source_image", "") : "projects/${lookup(each.value, "source_image_project", var.source_image_project)}/global/images/family/${lookup(each.value, "source_image_family", var.source_image_family)}"
